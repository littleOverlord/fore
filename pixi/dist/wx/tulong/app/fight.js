...
export class FScene {
  ...
  checkResult() {
    ...
    this.fighters.forEach(f => {
      if (f.camp === 1 && f.hp > 0) left++;
      if (f.camp === 0 && f.hp > 0) right++;
    });
...
class Policy {
   ...
   */
  static move(f, t, s) {
    let dis = Math.abs(t.x - f.x),
        d = dis / f.attackDistance;
  ...
  static selectTargets(f, fighters) {
    const camp = Math.abs(f.camp - 1),
          r = Policy.select(fighters, [["hp", ">", 0], ["camp", camp]]);
    r.sort((a, b) => {
      return a.x - b.x;
    });
    return r[0];
